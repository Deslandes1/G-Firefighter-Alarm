...
# ---------- EMAIL ALERT FUNCTION (using stored config) ----------
...
# ---------- JAVASCRIPT FOR ALARM SOUND (Web Audio) ----------
# This function returns HTML+JS that will start/stop an audible alarm.
def get_alarm_js(action):
    """
    action: "start" or "stop"
    Returns a JavaScript snippet to control the alarm.
    """
    if action == "start":
        return """
        <script>
            if (typeof window.alarmOscillator === 'undefined' || window.alarmOscillator === null) {
                try {
                    ...
                    oscillator.frequency.value = 880; // piercing sound
                    gain.gain.value = 0.5;
...
def main_app():
    ...
    # Logout button
    if st.button("Logout"):
        # Stop any playing alarm before logout
    ...
    if source == "Camera (Webcam)":
        if st.button("Start Detection"):
            ...
            st.session_state.email_sent = False
            st.session_state.alarm_playing = False
            # Ensure any previous alarm is off
            st.components.v1.html(get_alarm_js("stop"), height=0)
        ...
        if uploaded_file is not None:
            ...
            if fire_detected:
                ...
                # Play alarm for static image detection as well
                st.components.v1.html(get_alarm_js("start"), height=0)
